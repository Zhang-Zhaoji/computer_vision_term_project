...
class Model(nn.Module):
    ...
    def forward(self, content_images, style_images, alpha=1.0, lam=10, beta=10):
        content_features = self.vgg_encoder(content_images, output_last_feature=True)
        style_features = self.vgg_encoder(style_images, output_last_feature=True)
        t = adain(content_features, style_features)
        t = alpha * t + (1 - alpha) * content_features
        ...
        out_lap = self.lap_net(out)
        content_lap = self.lap_net(content_images)
        lap_loss = self.calc_content_loss(out_lap, content_lap)
        ...
        loss = loss_c + lam * loss_s +  lap_loss
        # loss = loss_c + lam * loss_s
        # print(f"loss_c: {loss_c}, loss_s: {loss_s}, lap_loss: {lap_loss}")
        return loss
