...
def main():
    ...
    parser.add_argument('--model_state_path', type=str, default='/hy-tmp/result_mono/model_state/9_epoch.pth',
                        help='save directory for result and loss')

    args = parser.parse_args()

    content_dir = "/root/test_content/"
    style_dir = "/root/test_style/"
    res_dir = "/root/res_mono"
    ...
    content_files = [os.path.join(content_dir, file) for file in os.listdir(content_dir)]
    style_files = [os.path.join(style_dir, file) for file in os.listdir(style_dir)]

    for content_image in content_files:
        for style_image in style_files:
            args.output_name = None
            c = Image.open(content_image)
            ...
            if args.output_name is None:
                c_name = os.path.splitext(os.path.basename(content_image))[0]
                s_name = os.path.splitext(os.path.basename(style_image))[0]
                args.output_name = f'{res_dir}/{c_name}_{s_name}'
